...
export const dashboardService = {
  ...
  async getPolicyDistribution(customerId?: string): Promise<Array<{ name: string; value: number; color: string }>> {
    ...
    const colors: Record<string, string> = { auto: '#3B82F6', home: '#10B981', life: '#8B5CF6', health: '#F59E0B' };
  ...
  async getClaimsOverview(customerId?: string): Promise<Array<{ name: string; value: number; color: string }>> {
    ...
    const colors: Record<string, string> = { submitted: '#6B7280', under_review: '#F59E0B', approved: '#10B981', rejected: '#EF4444', settled: '#3B82F6' };
    const labels: Record<string, string> = { submitted: 'Submitted', under_review: 'Under Review', approved: 'Approved', rejected: 'Rejected', settled: 'Settled' };
    return Object.entries(dist).map(([key, value]) => ({
      name: labels[key] || key,
      value,
      color: colors[key] || '#6B7280',
